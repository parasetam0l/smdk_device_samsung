...
static int amplifier_open(void)
{
    ...
    if (rc) {
        ...
        ALOGI("%s: Failed to obtain reference to amplifier module: %s\n",
    ...
    if (rc) {
        ALOGI("%s: Failed to open amplifier hardware device: %s\n",
                __func__, strerror(-rc));
        return -ENODEV;
    }
...
static snd_device_t get_output_snd_device(struct audio_device *adev, audio_devices_t devices)
{
    ...
    ALOGI("%s: enter: output devices(%#x), mode(%d)", __func__, devices, mode);
                             ...
                             snd_device_t snd_device)
{
    ...
    if (snd_device == SND_DEVICE_OUT_SPEAKER_AND_HEADPHONES) {
        ALOGI("Request to enable combo device: enable individual devices\n");
    ...
    if (adev->snd_dev_ref_cnt[snd_device] > 1) {
        ALOGI("%s: snd_device(%d: %s) is already active",
              __func__, snd_device, snd_device_name);
        return 0;
    }

    ALOGI("%s: snd_device(%d: %s)", __func__,
                              ...
                              snd_device_t snd_device)
{
    ...
    if (snd_device == SND_DEVICE_OUT_SPEAKER_AND_HEADPHONES) {
        ALOGI("Request to disable combo device: disable individual devices\n");
                                              ...
                                              snd_device_t snd_device)
{
    ...
    list_for_each(node, &adev->usecase_list) {
        ...
        if (usecase_snd_device != snd_device) {
            ALOGI("%s: Usecase (%s) is active on (%s) - disabling ..",
                          ...
                          audio_usecase_t uc_id)
{
    ...
    ALOGI("%s: usecase(%d)", __func__, uc_id);
    ...
    ALOGI("%s: out_snd_device(%d: %s) in_snd_device(%d: %s)", __func__,
                              ...
                              struct echo_reference_buffer *buffer)
{
    ...
    ALOGIV("get_capture_delay_time_stamp Secs: [%10ld], nSecs: [%9ld], kernel_frames:[%5d],"
...
{
    ALOGIV("%s: enter:), in->config.channels(%d)", __func__,in->config.channels);
    ...
    ALOGIV("update_echo_reference, in->config.channels(%d), frames = [%zd], in->ref_buf_frames = [%zd],  "
          "b.frame_count = [%zd]",
    ...
    if (in->ref_buf_frames < frames) {
        if (in->ref_buf_size < frames) {
            ...
            ALOGIV("update_echo_reference(): ref_buf %p extended to %d bytes",
        ...
        if (in->echo_reference->read(in->echo_reference, &b) == 0)
        {
            ...
            ALOGIV("update_echo_reference(): in->ref_buf_frames:[%zd], "
                                                      ...
                                                      uint32_t sampling_rate)
{
    ...
    if (adev->primary_output!= NULL && adev->primary_output->usecase == USECASE_AUDIO_PLAYBACK &&
        ...
        ALOGI("Calling create_echo_reference");
                       ...
                       struct echo_reference_buffer *buffer)
{
    ...
    if (status < 0) {
        ...
        buffer->time_stamp.tv_nsec = 0;
        buffer->delay_ns           = 0;
        ALOGI("get_playback_delay(): pcm_get_htimestamp error,"
    ...
    buffer->delay_ns = (long)(((int64_t)(kernel_frames + frames)* 1000000000)/
                            out->config.rate);
    ALOGIV("get_playback_delay_time_stamp Secs: [%10ld], nSecs: [%9ld], kernel_frames: [%5u], delay_ns: [%d],",
...
static int in_configure_reverse(struct stream_in *in)
{
    ...
    int i;
    ALOGI("%s: enter: in->num_preprocessors(%d)", __func__, in->num_preprocessors);
    if (in->num_preprocessors > 0) {
        ...
        for (i = 0; i < in->num_preprocessors; i++)
        {
            ...
            fct_status = (*(in->preprocessors[i].effect_itfe))->command(
                                                        ...
                                                        &size,
                                                        &cmd_status);
            ALOGI("%s: calling EFFECT_CMD_SET_CONFIG_REVERSE",__func__);
...
static int in_reconfigure_channels(struct stream_in *in,
    ...
    ALOGI("in_reconfigure_channels(): config_changed %d effect %p",
          config_changed, effect);

    /* if config changed, reconfigure all previously added effects */
    if (config_changed) {
        int i;
        ALOGI("%s: config_changed (%d)", __func__, config_changed);
        for (i = 0; i < in->num_preprocessors; i++)
        {
            int cur_status = in_configure_effect_channels(in->preprocessors[i].effect_itfe,
                                                  channel_config);
            ALOGI("%s: in_configure_effect_channels i=(%d), [main_channel,aux_channel]=[%d|%d], status=%d",
                          __func__, i, channel_config->main_channels, channel_config->aux_channels, cur_status);
            if (cur_status != 0) {
                ALOGI("in_reconfigure_channels(): error %d configuring effect "
                                   ...
                                   effect_handle_t effect)
{
    ...
    if (status != 0) {
        ALOGI("in_update_aux_channels(): in_reconfigure_channels error %d", status);
        /* resetting aux channels configuration */
        aux_channels = 0;
        channel_config.aux_channels = 0;
        in_reconfigure_channels(in, effect, &channel_config, true);
    }
    ALOGI("%s: aux_channels=%d, in->aux_channels_changed=%d", __func__, aux_channels, in->aux_channels_changed);
...
static ssize_t read_frames(struct stream_in *in, void *buffer, ssize_t frames)
{
    ...
    while (frames_wr < frames) {
        ...
        ALOGIV("%s: frames_rd: %zd, frames_wr: %zd, in->config.channels: %d",
...
static int stop_input_stream(struct stream_in *in)
{
    ...
    ALOGI("%s: enter: usecase(%d: %s)", __func__,
...
static int start_input_stream(struct stream_in *in)
{
    ...
    ALOGI("%s: enter: usecase(%d)", __func__, in->usecase);
     ...
     */
    ALOGI("%s: Opening PCM device card_id(%d) device_id(%d), channels %d, smp rate %d format %d, \
...
error_config:
    ALOGI("%s: exit: status(%d)", __func__, ret);
    adev->active_input = NULL;
    return ret;
}
...
static int out_open_pcm_devices(struct stream_out *out)
{
    ...
    list_for_each(node, &out->pcm_dev_list) {
        ...
        ALOGI("%s: Opening PCM device card_id(%d) device_id(%d)",
...
static int start_output_stream(struct stream_out *out)
{
    ...
    ALOGI("%s: enter: usecase(%d: %s) devices(%#x) channels(%d)",
...
int stop_hfp_call(struct audio_device *adev)
{
    struct audio_usecase *uc_info;
...
int start_hfp_call(struct audio_device *adev)
{
    struct audio_usecase *uc_info;
    int ret = 0;
    ...
    if (out->usecase != USECASE_AUDIO_PLAYBACK_OFFLOAD) {
        ...
        if (out->echo_reference != NULL) {
            ...
            if (out->echo_reference_generation != adev->echo_reference_generation) {
                ALOGI("%s: release_echo_reference %p", __func__, out->echo_reference);
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    ALOGI("%s: enter: usecase(%d: %s) kvpairs: %s out->devices(%#x) "
          "adev->mode(%#x)",
    ...
    if (ret >= 0) {
        ...
        ALOGI("%s: routing: usecase(%d: %s) devices=(%#x) adev->mode(%#x)",
...
static char* out_get_parameters(const struct audio_stream *stream, const char *keys)
{
    ...
    int ret;
    bool first = true;
    ALOGI("%s: enter: keys - %s", __func__, keys);
    ...
    ALOGI("%s: exit: returns - %s", __func__, str);
    return str;
}
    ...
    if (out->standby) {
        ...
        if (adev->active_input &&
            (adev->active_input->source == AUDIO_SOURCE_VOICE_COMMUNICATION ||
                adev->active_input->source == AUDIO_SOURCE_MIC)) {
                    in = adev->active_input;
                    ALOGI("%s: enter:) force_input_standby true", __func__);
        }
#endif
        pthread_mutex_unlock(&adev->lock);
#ifdef PREPROCESSING_ENABLED
...
false_alarm:
    ...
    if (out->usecase == USECASE_AUDIO_PLAYBACK_OFFLOAD) {
        ...
        if (android_atomic_acquire_load(&adev->echo_reference_generation)
            ...
            out->echo_reference_generation = adev->echo_reference_generation;
            out->echo_reference = adev->echo_reference;
            ALOGI("%s: update echo reference generation %d", __func__,
                  out->echo_reference_generation);
            pthread_mutex_unlock(&adev->lock);
        }
#endif
...
#endif
                ALOGIV("%s: writing buffer (%d bytes) to pcm device", __func__, bytes);
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    ALOGI("%s: enter: kvpairs=%s", __func__, kvpairs);
    parms = str_parms_create_str(kvpairs);
...
exit:
    ...
    if (read_and_process_successful == false) {
        ...
        ALOGI("%s: read failed - sleeping for buffer duration", __func__);
        struct timespec t = { .tv_sec = 0, .tv_nsec = 0 };
        clock_gettime(CLOCK_MONOTONIC, &t);
        const int64_t now = (t.tv_sec * 1000000000LL + t.tv_nsec) / 1000;
    ...
    if ( enable == true ) {
        ...
        for (i=0; i < in->num_preprocessors; i++) {
            if (status == 0) { /* status == 0 means an effect was removed from a previous slot */
                ...
                ALOGI("add_remove_audio_effect moving fx from %d to %d", i, i-1);
                continue;
            }
            if ( in->preprocessors[i].effect_itfe == effect ) {
                ALOGI("add_remove_audio_effect found fx at index %d", i);
        ...
        ALOGI("%s: enable(%d), in->aux_channels_changed(%d)", __func__, enable, in->aux_channels_changed);
    }
    ALOGI("%s:  num_preprocessors = %d", __func__, in->num_preprocessors);

    if ( memcmp(&desc.type, FX_IID_AEC, sizeof(effect_uuid_t)) == 0) {
        in->enable_aec = enable;
        ALOGI("add_remove_audio_effect(), FX_IID_AEC, enable: %d", enable);
        if (!in->standby) {
            select_devices(in->dev, in->usecase);
            do_in_standby_l(in);
        }
...
static int in_add_audio_effect(const struct audio_stream *stream,
                               effect_handle_t effect)
{
    ALOGI("%s: effect %p", __func__, effect);
                                   ...
                                   const char *address __unused)
{
    ...
    ALOGI("%s: enter: sample_rate(%d) channel_mask(%#x) devices(%#x) flags(%#x)",
    ...
    *stream_out = NULL;
    ...
    if (out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) {
        ...
        ALOGI("%s: use AUDIO_PLAYBACK_DEEP_BUFFER",__func__);
    } else {
        out->usecase = USECASE_AUDIO_PLAYBACK;
        out->sample_rate = out->config.rate;
    }
    ...
    *device = NULL;
    ...
    if (access(OFFLOAD_FX_LIBRARY_PATH, R_OK) == 0) {
        ...
        if (adev->offload_fx_lib == NULL) {
            ...
            ALOGI("%s: DLOPEN successful for %s", __func__, OFFLOAD_FX_LIBRARY_PATH);
