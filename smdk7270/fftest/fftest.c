...
int main(int argc, char** argv)
{
	...
	effects[0].replay.length = 5000;
	effects[0].replay.delay = 1000;

	printf("Uploading effect #0 (Strong rumble, with heavy motor) ... ");
	fflush(stdout);
	...
	effects[1].u.rumble.weak_magnitude = 0xc000;
	effects[1].replay.length = 5000;
	effects[1].replay.delay = 0;

	printf("Uploading effect #1 (Weak rumble, with light motor) ... ");
	fflush(stdout);
