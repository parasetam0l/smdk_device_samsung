...
bool AccelSensor::hasPendingEvents() const {
    ...
    ALOGE("AccelSensor::~hasPendingEvents %d", mHasPendingEvent ? 1 : 0 );
...
int AccelSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    fd = open(SSP_ACCEL_POLL_DELAY_PATH, O_RDWR);
    if (fd >= 0) {
        char buf[80];
        sprintf(buf, "%lld", ns);
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_REL) {
            ...
            if (event->code == EVENT_TYPE_ACCEL_X) {
                ...
                ALOGE("AccelSensor::readEvents X: %d", mPendingEvent.acceleration.x);
            } else if (event->code == EVENT_TYPE_ACCEL_Y) {
                mPendingEvent.acceleration.y = value * CONVERT_A_Y;
                ALOGE("AccelSensor::readEvents Y: %d", mPendingEvent.acceleration.y);
            } else if (event->code == EVENT_TYPE_ACCEL_Z) {
                mPendingEvent.acceleration.z = value * CONVERT_A_Z;
                ALOGE("AccelSensor::readEvents Z: %d", mPendingEvent.acceleration.z);
            }
        } else if (type == EV_SYN) {
